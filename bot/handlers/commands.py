...
_HELP_TEXT = (
    ...
    "• Или запишите голосовое — распознаю и отвечу (покажу расшифровку для проверки).\n"
...
# (короткая подпись кнопки, полный вопрос в агента) — Telegram обрезает длинный
# текст кнопки, поэтому на кнопке короткий label, а в LLM уходит полный вопрос.
TOPICS: dict[str, tuple[str, list[tuple[str, str]]]] = {
    "labor": ("💼 Трудовые", [
        ("Увольнение в отпуске", "Могут ли уволить во время отпуска?"),
        ("Сколько дней отпуска", "Сколько дней ежегодного отпуска положено?"),
        ("Выплаты при сокращении", "Что положено работнику при сокращении?"),
        ("Испытательный срок", "Какой максимальный испытательный срок по закону?"),
    ]),
    "family": ("👪 Семейные", [
        ("Как подать на развод", "Как подать на развод?"),
        ("Размер алиментов", "Как рассчитываются алименты на ребёнка?"),
        ("Раздел имущества", "Как делится имущество супругов при разводе?"),
    ]),
    "consumer": ("🛒 Права потребителя", [
        ("Возврат некачественного", "Как вернуть некачественный товар?"),
        ("Срок возврата товара", "В какой срок можно вернуть товар?"),
        ("Не возвращают деньги", "Что делать, если не возвращают деньги за товар?"),
    ]),
    "housing": ("🏠 Жильё и ЖКХ", [
        ("Затопил сосед", "Что делать, если затопил сосед сверху?"),
        ("Приватизация квартиры", "Как приватизировать квартиру?"),
        ("Капремонт дома", "Кто отвечает за капитальный ремонт дома?"),
    ]),
    "traffic": ("🚗 Штрафы и ДТП", [
        ("Оспорить штраф ГИБДД", "Как оспорить штраф ГИБДД?"),
        ("Действия при ДТП", "Что делать при ДТП без пострадавших?"),
        ("Выплата по ОСАГО", "Как получить выплату по ОСАГО?"),
    ]),
    "money": ("💳 Кредиты и долги", [
        ("Досрочное погашение", "Можно ли досрочно погасить кредит без штрафа?"),
        ("Приставы и карта", "Что делать, если приставы арестовали карту?"),
        ("Банкротство физлица", "Как объявить себя банкротом?"),
    ]),
}
...
def _topic_questions_keyboard(topic_id: str) -> InlineKeyboardMarkup:
    ...
    rows = [
        [InlineKeyboardButton(text=label, callback_data=f"tq:{topic_id}:{i}")]
        for i, (label, _q) in enumerate(questions)
    ]
...
async def on_topic_question(
    ...
    if not (0 <= idx < len(questions)):
        await callback.answer("Вопрос не найден")
        return
    question = questions[idx][1]  # полный вопрос (label — только для кнопки)
