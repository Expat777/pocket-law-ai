...
- После пересборки образа (PR #21) сценарий документ→ответ **подтверждён в живом Telegram**: ссылка ст.115 → «28 календарных дней» (ст.115/267/322); полный ТК → вопрос про увольнение → grounded-ответ (ст.81/180/296/318/394). Документ реально влияет на ответ, цитаты кликабельны. **Все задачи Роли 1 закрыты.**
