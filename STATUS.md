...
- **Роль 1:** фикс документа подтверждён на агенте live (документ→ответ работает); осталось только ручное подтверждение в Telegram.
...
- После пересборки образа (PR #21) сквозной прогон документ→ответ подтверждён на боевой модели `gemini-2.5-flash-lite`: ссылка ст.115 → «28 календарных дней» + цитаты ст.115/267/322/122/321 (изолированный вызов агента). Хвост закрыт на уровне агента.
